...
import { commitPersona, commitDirecciones, commitTelefonos } from "./commit";

let nextDireccionId = 0;
let nextTelefonoId = 0;

function AgregarPersona({ onClose, onGuardado }) {
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      await commitPersona(formData, null, (p) => { nuevaPersona = p; });

      if (nuevaPersona?.id) {
        await commitDirecciones(direcciones, nuevaPersona.id);
        await commitTelefonos(telefonos, nuevaPersona.id);
      }

      if (onGuardado) onGuardado(nuevaPersona);
      onClose();
    } catch (error) {
